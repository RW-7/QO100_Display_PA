...
#include "CIVcmds.h"
	...
	uint8_t bd_address[6]  = {0x30, 0x31, 0x7D, 0x34, 0x89, 0xB0};
#endif
...
{
  CIV_BTSER.register_callback(callback);

  // Setup bluetooth as master:
  if(!CIV_BTSER.begin("T1-INTERFACE",true)) {
  ...
  btConnected = CIV_BTSER.connect(bd_address);
